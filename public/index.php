...
require_once __DIR__ . '/../app/Controllers/CalendarExportController.php';
...
require_once __DIR__ . '/../app/Models/NotificationLog.php';
...
$calendarExportC = new CalendarExportController();
...
elseif ($path === '/admin/reports/export/pdf' && $method === 'GET') {
    $reportC->exportPdf();
}
elseif ($path === '/admin/reports/export/excel' && $method === 'GET') {
    $reportC->exportExcel();
}
...
elseif ($path === '/admin/notifications' && $method === 'POST') {
    $notificationC->update();
}
elseif ($path === '/admin/notifications/test' && $method === 'POST') {
    $notificationC->sendTest();
}

// ── Admin calendar export ───────────────────────────────────────
elseif ($path === '/admin/calendar-export' && $method === 'GET') {
    $calendarExportC->index();
}
elseif ($path === '/admin/calendar-export/download' && $method === 'GET') {
    $calendarExportC->download();
}
...
elseif (preg_match('#^/bookings/(\d+)/ical$#', $path, $m) && $method === 'GET') {
    $bookingC->downloadIcal((int)$m[1]);
}
elseif (preg_match('#^/bookings/series/(\d+)/ical$#', $path, $m) && $method === 'GET') {
    $bookingC->downloadSeriesIcal((int)$m[1]);
}
